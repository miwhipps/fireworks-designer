...
const FountainEffect = ({ color, shouldInitialize, isEmitting = true }) => {
  ...
  useEffect(() => {
    if (!shouldInitialize) {
      accumulatedTimeRef.current = 0;
      // Clear particles when scrubbing backwards out of fountain range
      particlesRef.current = [];
      geometryInitializedRef.current = false;
    }
  }, [shouldInitialize]);
  ...
  useFrame((_, delta) => {
    ...
    // Continue updating existing particles even when not initializing
    if (!shouldInitialize && !particlesRef.current.length) return;
    
    // Only accumulate time when emitting
    if (isEmitting) {
    ...
    if (isEmitting) {
      ...
      while (accumulatedTimeRef.current >= spawnInterval) {
        const direction = generateRandomInCone(CONE_ANGLE);
    ...
    if (particlesRef.current.length > 0 && geometryRef.current && geometryInitializedRef.current) {
      ...
      particlesRef.current.forEach((particle, i) => {
        ...
        const alpha = getRealisticParticleAlpha(particle, 'fountain');
        const [r, g, b] = getFireworkColor(color, alpha, 'fountain', brightnessMultiplier);

        colorArray[i * 3] = r;
        colorArray[i * 3 + 1] = g;
        colorArray[i * 3 + 2] = b;
        
        sizeArray[i] = particle.size * alpha * 20 * Math.sqrt(brightnessMultiplier);
      });
